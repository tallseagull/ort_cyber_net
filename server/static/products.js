...
var app = {
  ...
  renderTemplates: function (search_str) {
    "use strict";
...
const form  = document.getElementById('run-search');

form.addEventListener('submit', (event) => {
    // handle the form data - prevent the form submission
    event.preventDefault();
    app.renderTemplates(form.elements['search_str'].value);
});

// Listen to the 'checkout' button:
document.getElementById("checkout_btn").addEventListener("click", function() {
  // Get the selected items, and write back to the server:
  // Loop over the products, find the ones selected and add their product-id to our list:
    var products = $(".shopping-cart--list-item"),
        selected = [];

    for (var i = 0; i < products.length; i += 1) {
      if (parseFloat( $(products[i]).find(".product-qty").html() ) > 0) {
        selected.push($(products[i]).find(".product-id").html())
      }
    }
    console.log("Found selected:")
    console.log(selected)

  $.post(base_path + '/checkout',
      {'selected': selected},
      function (response){
          // Get us to the 'thank you' page:
          location.href = base_path + "/thank_you"
      })
});
